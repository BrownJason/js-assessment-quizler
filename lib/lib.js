...
exports.createQuestions = exports.createPrompt = exports.chooseRandom = exports.writeFileSync = exports.readFileSync = undefined;
...
const readFileSync = fileName => new Promise((resolve, reject) => {
  _fs2.default.readFile(_path2.default.resolve(__dirname, fileName), (err, data) => {
    err ? reject(Error(err)) : resolve(JSON.parse(data));
  });
});
// TODO copy writeFile() from exercise
const writeFileSync = (fileName, data) => new Promise((resolve, reject) => {
  _fs2.default.writeFile(_path2.default.resolve(__dirname, fileName), JSON.stringify(data), err => err ? reject(Error(err)) : resolve('File save succesfully'));
});
...
const chooseRandom = (array = [], numItems) => {
  ...
  const uniqueIndex = (res, next = Math.floor(Math.random() * array.length)) => {
    return res.includes(next) ? uniqueIndex(res, Math.floor(Math.random() * array.length)) : [...res, next];
  };

  const randomIndices = Array(numItems >= 1 && numItems <= array.length ? numItems : Math.floor(Math.random() * array.length)).fill().reduce(uniqueIndex, []);
...
const createQuestions = (obj = {}) => {
  ...
  */
  ...
  let qName = 0;
  const questionArray = Array(objArrayLength.length).fill().reduce((res, next) => {
    qName++;
